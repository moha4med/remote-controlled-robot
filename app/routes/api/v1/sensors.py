...
def environment():
    ...
    if not latest:
        return jsonify({"temperature": None, "humidity": None}), 404
...
def history():
    ...
    readings = (
        SensorLog.query
        .filter(SensorLog.recorded_at >= since)
...
def stats():
    ...
    return jsonify({
        ...
        "humidity": {
            "avg": round(result.avg_humidity, 1) if result.avg_humidity else None,
            "min": round(result.min_humidity, 1) if result.min_humidity else None,
            "max": round(result.max_humidity, 1) if result.max_humidity else None,
        },
        "battery": {
            "avg": round(result.avg_battery, 1) if result.avg_battery else None,
        },
        "signal": {
            "avg": round(result.avg_signal, 1) if result.avg_signal else None,
        },
    })